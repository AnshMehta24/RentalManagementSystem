...
export default function EditProductForm({
  ...
  const selectedAttributesRaw = watch("selectedAttributes");
  ...
  useEffect(() => {
    ...
    const detectedVariantMode = hasAttributes ? "variants" : "default";

    let formData: ProductFormValues;

    if (detectedVariantMode === "variants" && variants.length > 0) {
      const selectedAttrIds = new Set<number>();
      const attrValueMap: Record<number, number[]> = {};

      variants.forEach((variant) => {
        variant.attributes?.forEach((attr) => {
          selectedAttrIds.add(attr.attributeId);
          if (!attrValueMap[attr.attributeId])
            attrValueMap[attr.attributeId] = [];
          if (!attrValueMap[attr.attributeId].includes(attr.attributeValueId)) {
            attrValueMap[attr.attributeId].push(attr.attributeValueId);
          }
        });
      });
      ...
      const formVariants = variants.map((variant) => ({
        ...
        attributeValues: variant.attributes.map((a) => ({
          attributeId: a.attributeId,
          attributeValueId: a.attributeValueId,
        })),
        rentalPrices: rentalPeriods.map((period) => ({
          periodId: period.id,
          price:
            variant.prices.find((p) => p.periodId === period.id)?.price || 0,
        })),
      }));
      ...
      formData = {
        ...
        selectedAttributes: Array.from(selectedAttrIds),
        variants: formVariants,
      };

      setSelectedAttributeValues(attrValueMap);
    } else {
      const defaultVariant = variants[0];
      const rentalPrices = rentalPeriods.map((period) => {
        const existingPrice = defaultVariant.prices.find(
  ...
  useEffect(() => {
    ...
    if (variantMode === "default") {
      ...
      const periods = rentalPeriods.map((p) => ({
        periodId: p.id,
        price: 0,
      }));
      const currentDefault = watch("defaultVariant");
      if (!currentDefault) {
            ...
            <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm space-y-6">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-start gap-4">
                  <div className="relaive w-40 h-40 rounded-lg border-2 border-dashed border-gray-200 bg-gray-50 flex items-center justify-center overflow-hidden">
                    {watch("imageUrl") ? (
                      <>
                        <Image
                          height={100}
                      ...
                      >
                        <div className="flex items-center justify-between">
                          ...
                          <button
                            ...
                            className="px-4 py-2 rounded-md border border-gray-300 bg-white text-sm font-medium hover:bg-gray-50 transition-colors ml-4"
                          >
                            Configure
                          </button>
                        </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      ...
                      {errors.defaultVariant && (
                        <div className="mt-2 space-y-1">
                          {errors.defaultVariant.quantity?.root?.message && (
                            <p className="text-xs text-red-600">
                              {
                                errors.defaultVariant.quantity.root
                                  .message as string
                              }
                            </p>
                          )}
                          {errors.defaultVariant.costPrice?.root?.message && (
                            <p className="text-xs text-red-600">
                              {
                                errors.defaultVariant.costPrice.root
                                  .message as string
                              }
                            </p>
                          )}
                          {errors.defaultVariant.salePrice?.root?.message && (
                            <p className="text-xs text-red-600">
                              {
                                errors.defaultVariant.salePrice.root
                                  .message as string
                              }
                            </p>
                          )}
                          {errors.defaultVariant.rentalPrices?.root
                            ?.message && (
                            <p className="text-xs text-red-600">
                              {
                                errors.defaultVariant.rentalPrices.root
            ...
            <div className="sticky top-6 space-y-6">
              ...
              <div className="flex flex-col gap-3">
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full rounded-md bg-blue-600 px-6 py-3 text-sm font-medium text-white hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
                >
                  {loading ? "Updating..." : "Update Product"}
                </button>
                <Link
                  href="/vendor/products"
                  className="w-full text-center rounded-md border border-gray-300 bg-white px-6 py-3 text-sm font-medium text-gray-900 hover:bg-gray-50 transition-colors"
                >
                  Cancel
                </Link>
              </div>
